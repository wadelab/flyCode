...
for thisFlyTypeIndex=1:nUniqueFlies
    allData{thisFlyTypeIndex}=allExptResponses(find(uniqueJ==thisFlyTypeIndex),:,:);
...
% 
%%%***** TODO
nContLevels=params.nTrials;

for thisPhenotype=1:length(ptypeList)
     nMaskLevels=size(allData{thisPhenotype},3)/nContLevels;
     nFlies=size(allData{thisPhenotype},1);
     nExtractedFreqs=size(allData{thisPhenotype},2);
     
     analysisStruct.allFlyDataCoh{thisPhenotype}=reshape(allData{thisPhenotype},[nFlies,nExtractedFreqs,nContLevels,nMaskLevels]); % We make this analysis struct with the same data format for the old and new rigs so that we can analyze all our data in one place...         
     analysisStruct.allNormFlyDataCoh{thisPhenotype}=[];;
          
     analysisStruct.params{thisPhenotype}=params;
     analysisStruct.phenotypeName{thisPhenotype}=ptypeList{thisPhenotype};
     analysisStruct.nFlies{thisPhenotype}=size(allData{thisPhenotype},1);
     
     analysisStruct.avWaveform{thisPhenotype}=[]; % For now we don't save the average waveform
...
save(aFileName,'analysisStruct'); % You can stop here...
