    ...
    if isfield(phenotypeList{thisPhenotype}, 'flyDir')
        ...
        if nFlies > 0
            ...
            for thisFlyIndex=1:nFlies
                if ~isfield(phenotypeList{thisPhenotype}.flyData{thisFlyIndex},'fileNames') % This directory has no data in it
                    errorstring=sprintf('\nNo data found in directory %s',phenotypeList{thisPhenotype}.flyDir{thisFlyIndex});
                    warning(errorstring);
                    ...
                    condDat=[];
                    meanSpect=[];
                    for thisFile=1:nDataSets % 2:nDataSets % Skip the first one - light adaptation!
                    ...
                    fCondDat=(fft(condDat)/sqrt(size(avWaveform,1))*1000); % Compute FT down time in each bin. Leave in complex domain
