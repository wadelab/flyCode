...
% ARW 051213
% This script does frequency analysis on Olivia's data
% It looks at per-day cycles and does some FFTs to pull out which F's are
% present.
...
xwins = 3; ywins = 6; % This is for Olivia - 6 time points per day
%count_subwins(maxPhenotypesToPlot);
figure();
...
%% From here on we do something different
% Olivia is measuring circadian Rhythms. We would like to fit the data with
% some sort of periodic function to see if it's locked to a 24 hour cycle.
% One way to do this is to do an FT and look at the 1st harmonic.
% We can bootstrap this so that we get an idea of the prbability
% distribution
% Some things to think about
% 1: There is probably a toolbox in R that does this. I don't know. It is
% worth looking.
% 2: Other methods have been published. However, bootstrapping from sampled
% data seems to be an idea that others have tried.


% We will look at the 1F1 Rmax values for now. These are clearly periodic
% in, say, the ClockJerk mutants.

%%
%rmParams=squeeze(fittedCRFParams(:,1,1,1));
rmParams=squeeze(fittedCRFParams(:,1,1,1));
size(rmParams) 
figure(12);
subplot(2,1,1);
...
plot(rmParams);
set(gca,'YLim',[0 .05]);
% First, let's, for fun, look at the FT of the whole tSeries. Y scale for
% both figures altered for jrk st1 data
ftRM=fft(rmParams);
xlabel('Rmax Timecourse over whole data period (3days)');

subplot(2,1,2);
bar(abs(ftRM(2:9)));
set(gca,'YLim',[0 .05]);
xlabel('FT over whole data period (3days) with mean removed');

% Now look at the three days separately

dayRM=reshape(rmParams,6,3);
dayFT=fft(dayRM);
%%
figure(13);
subplot(2,1,1);
...
% Now... Do some bootstrapping. We will shuffle the order of everything
% lots of times and look for how special the 1st harmonic is...
% I think a cool thing to do is bootstrap the 1st cycle amplitudes
