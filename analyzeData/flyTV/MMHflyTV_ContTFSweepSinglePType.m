...
for thisFly=1:nFlies
    for thisRep=1:nRepeats
        ...
        for thisTrial=1:nTrials
            fullDat(thisFly,thisRep,tfIndexIn(thisTrial),contIndexIn(thisTrial),:)=dataStruct(thisFly,thisRep,thisTrial,:);
        end
...
%Digitizer converts the analog signal to a digital one for us to analyse
if (digitizerRate ~= fix(digitizerRate))
    fprtinf('\nWarning!Non integer digitizer rate %.2d \nSetting to 1000 but beware!\n',digitizerRate);
    digitizerRate=1000;
end

%crop the full data(fly, rep, tf, cont) to remove the first second
...
%the ifft (so FT then strip out 48-52 as this is the input frequency, then
%ifft to get the waveform back).
...
% HERE - Plot some time courses

%[4 5] will show us 6hz and 8hz. reference - 1=1, 2=2, 3=4, 4=6, 5=8, 6=12, 7=18, 8=36hz
egSeq=croppedDat(:,1,[4 5],8,:); %5D Array, 
egSeq=squeeze(egSeq); %removes singularities
...
fOverall=fft(overallSeq); %ftransform 
fOverall(1,:)=0; % Remove the mean so that it plots nicely
fOverall(48:52,:)=0; %stripping out the line noise
fOverall((end-51):(end-48),:)=0;
reconSeq=real(ifft(fOverall)); % inverse ft to get it back
%%
figure(99);
subplot(2,1,1);
c=plot(reconSeq);
...
%for std and means for error bars 
singlePmeans = squeeze(abs(mean(exAmps(:,:,:,2))));
singlePAngle = squeeze(angle(mean(exAmps(:,:,:,2))));

singlePSEM = squeeze(abs(std(exAmps(:,:,:,2))))/sqrt(nFlies);

%% Perform PCA on the data to see what the PCs look like
dataToPCA=squeeze(exAmps(:,:,:,2));
% Reformat it so that variables are columns and rows are obs, as required for PCA..
dataToPCAformat=reshape(dataToPCA,[nFlies,64]);

[pcComps,pcScores]=pca(dataToPCAformat);

% Loop over the first 4 components, showing them as pics.
figure(111);
for thisComp=1:10
    subplot(2,5,thisComp);
    imagesc(reshape(abs(pcComps(:,thisComp)),8,8));
    colormap hot;
    title(int2str(thisComp));
...
figure(11);
subplot(2,1,1);
imagesc(log(squeeze(abs(mean(exAmps(:,:,:,2)))))); % Can also compute SEMs using std
xlabel('Contrast', 'FontSize', 15);
ylabel('Temporal Frequency (Hz)', 'FontSize', 15);
colormap('hot');
c = colorbar;
ylabel(c, 'F2 amplitude microVolts')
set(gca,'Xtick',1:8,'XTickLabel',{'1%', '2%', '4%', '8%', '16%', '32%', '64%', '99%'})
set(gca,'Ytick',1:8,'YTickLabel',{'1', '2', '4', '6', '8', '12', '18', '36'})

title('Mean coherence across flies');
%contrast response functions - TO DO: GET SD IN
subplot(2,1,2);
allConts=unique(contList);
plot(allConts,squeeze(abs(mean(exAmps(:,:,:,2))))'); %2 is the harmonic
set(gca,'YScale','Linear');
hleg = legend('1','2','4','6','8','12','18','36');
...
figure(12);
subplot(2,1,1);
errorbar(abs(singlePmeans), abs(singlePSEM));
set(gca,'YLim',[0 .015]);
set(gca,'yscale','log')
cleg =legend({'1','2','4','8','16','32','64','99'});
set(gca,'Xtick',1:8,'XTickLabel',{'1', '2', '4', '6', '8', '12', '18', '36'})
ctitle = get(cleg,'Title');
set(ctitle,'String','Contrast (%)');
...
set(gca,'YLim',[0, .0060]);
set(gca,'yscale','log')
tleg=legend({'1','2','4','6','8','12','18','36'});
set(gca,'Xtick',1:8,'XTickLabel',{'1', '2', '4', '8', '16', '32', '64', '99'})
ttitle = get(tleg,'Title');
set(ttitle,'String','Temporal Frequency (Hz)');
xlabel('Contrast')
ylabel('Amps')

%% CLASSIFICATION ANALYSIS
%All the important data are now in extractedAmps - x
