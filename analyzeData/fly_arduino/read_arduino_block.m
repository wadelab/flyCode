...
function [thisFlyData, success] = read_arduino_file (block, fName, bCloseGraphs)
...
if (nContrasts ~= 45)
    thisFlyData.Error = ['Not exactly 45 contrasts in file : ', fName]
    success = false ;
    return
end

timedata = inputdata(1:1024,1);
timedata = timedata - timedata(1);

iStart = 1;
iEnd = 1024;
% one block so only 9 contrasts
nContrasts = 9 ;

figure ('Name', strcat('Rawdata of: ',fileName));
alldata = zeros(1025 * 9,3);

alldata = inputdata ( 1 + nContrasts * (block-1) * 1025: nContrasts * block * 1025,:);
...
printFilename = [pathstr, filesep, fileName, '_', num2str(block), '_RawData', sExt];
h=gcf;
...
printFilename = [pathstr, filesep, fileName, '_', num2str(block),  '_FFT', sExt];
h=gcf;
set(h,'PaperOrientation','landscape');
set(h,'PaperUnits','normalized');
set(h,'PaperPosition', [0 0 1 1]);
...
if (success)
    ...
    printFilename = [pathstr, filesep, fileName, '_', FreqNames{1}, '_', num2str(block), '_CRF', sExt];
    h=gcf;
    set(h,'PaperOrientation','landscape');
    set(h,'PaperUnits','normalized');
    set(h,'PaperPosition', [0 0 1 1]);
