...
datadir='C:\data\SSERG\data\SFMaxTest\';
flyTV_startTime=now;
...
tfList=[8;8]'%[1,2,4,6,8,12,18,36;1,2,4,6,8,12,18,36]'; % This is in Hz.
sfList=[.014,.028,.056,.11,.22,.44,.88,1.76,3.52;.014,.028,.056,.11,.22,.44,.88,1.76,3.52]'; % Cycles per degree
...
for thisRun=1:nRepeats  % 5 repeats
    for thisCond=1:nConds
        ...
        if (~DUMMYRUN)
            ...
            finalData.comment='wapr_1DPE_8Hz_10';
            finalData.stim=stim;
            finalData.now=now;
