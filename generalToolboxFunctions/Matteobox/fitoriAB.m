...
% figure; plot(xx,a1*xx,'k--',a2*yy,yy,'b-',xx,yy,'ro');
...
if isnan(fixedpars(3)) & max(initoo)>180
   rn 		= mean(rr(find(diffangles==max(diffangles))));
   minrn 	= 0;
   maxrn 	= max(rr);
elseif isnan(fixedpars(3)) & max(initoo)<=180
   rn       = rp;
   minrn 	= 0;
   maxrn 	= max(rr);
else
...
if isnan(fixedpars(5))
   ...
   sigma	= fixedpars(5);
   minsigma	= fixedpars(5)-(1/5 * fixedpars(5));
   maxsigma	= fixedpars(5)+(1/5 * fixedpars(5));
end
