        ...
        if goodfly(genotype, fly)
            ...
            for freq = 1 : freqmax
                for mask = 1 : maskmax
                   ...
                   % contrast = 5 ;
                    %calculate the response
                    response = 1000 * max(abs(analysisStruct.allFlyDataCoh{1,genotype}(fly,freq,:,mask)));
                    
                    
                    
                    outcells{iPreviousFlies, (freq * 2) + mask - 1} = { num2str(response) };
                end %if
            end;
