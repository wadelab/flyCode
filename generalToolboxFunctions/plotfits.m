...
for thisPhenotype=1:length(analysisStruct.nFlies)
    ...
    fittedNames{end + 1}= tmpName;
   
    fprintf('%s had %d flies \n', tmpName, analysisStruct.nFlies{thisPhenotype});
    if ( analysisStruct.nFlies{thisPhenotype} > 1 ) 
        fliesWithRealData = fliesWithRealData + 1 ;
    end
end

fprintf('\n\n\n');
%% sort names sanely
[sortednames,sortIndex] = sort(fittedNames);
for thisPhenotype=1:length(fittedNames)
    fprintf('%s had %d flies \n', fittedNames{sortIndex(thisPhenotype)}, analysisStruct.nFlies{sortIndex(thisPhenotype)});
end
%% 

%%Throw away singletons and photodiode
iNext = 1 ;
myRealBootData = [];
for i=1:length(sortIndex)
    if (sortIndex(i) < length(fittedNames))
        if (analysisStruct.nFlies{sortIndex(i)} > 2)
            if (~ strcmpi('Photodiode',sortednames(i)))
                tmp = bootFitParams(sortIndex(i),:,:,:,:);
                myRealBootData (iNext,:,:,:,:) = tmp ;
                plotNames(iNext) = sortednames(i) ;
                iNext = iNext + 1 ;
            end
        end
    end
...
%%
xToPlot = linspace(0,iNext,iNext);
if fliesWithRealData < 4 % try 4
    sStyle = 'horizontal' ;
    xAngle = 0 ;
else
    sStyle = 'inline' ; 
...
for iFreqComponent =1:2
    ...
    %figure;
    maxRMax = max(max(max(max(squeeze(myRealBootData(:,:,iFreqComponent,:,1))))));
    maxRMax = maxRMax * 1.2 ;
    for thisMaskCondition=1:2
       % sPlot = subplot(2,1,thisMaskCondition);
        d= myRealBootData(:,thisMaskCondition,iFreqComponent,:,1);
        dataToBoxplot=squeeze(d)';

        figure();
        boxplot(dataToBoxplot,'plotstyle','compact','labelorientation',sStyle,'notch','on','labels', plotNames); % The Rmax fits for both mask and unmasked
...
%% Plot Rmax mean etc ...   
    ...
    for thisMaskCondition=1:2
        sPlot = subplot(2,1,thisMaskCondition);
        dataToBoxplot=squeeze(myRealBootData(:,thisMaskCondition,iFreqComponent,:,1))';
        %notBoxPlot(dataToBoxplot); errorbar(mean(dataToBoxplot), std(dataToBoxplot));
...
%% Do the same for c50
   % to get the scale the same in each case... , and allow for headroom multiply by 1.2
    maxc50 = max(max(max(max(squeeze(myRealBootData(:,:,:,:,2)))))) ;
    maxc50 = maxc50 * 1.2 ;
for iFreqComponent =1:2
    %  We can plot these in boxplots like this:
    figure;
