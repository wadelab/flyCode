...
for thisSubDir=1:length(subDirList) % The concept of individual directories for different phenotypes is a little redundant now since all the ptype info is held in the file itself.
    ...
    for thisExptIndex=1:nExpts
        ...
        for thisFile=2:nDataSets % Load in all the files in the directory
           ...
           % condDat(:,:,:,:,thisFile)=rawPerFileData; % NB we are runnign out of memory here for some systems. We could average across bins immediately...
            ...
            avWaveformThisFile=squeeze(nanmean(rawPerFileData,2)); % Average down bins
                 
           
           % [sampleRate,binsPerTrial,nChannels,nTrials, files]
        ...
        meanDat=(squeeze(mean(fCondDat,4))); % was abs
